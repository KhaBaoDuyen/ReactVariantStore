...
import type { MainDropdownProps } from "./MainDropdown.type";
...
export const MainDropdown = ({
	...
	const hasDropdown =
		categories.length > 0 || brands.length > 0 || popularSearches.length > 0;

	return (
		<div className="relative group mx-4">
			<Link
				to={path}
				className="font-semibold flex items-center gap-1 py-4 hover:text-accent-600 transition-colors"
			>
				{title}
				{hasDropdown && (
					<ChevronDown
				...
				>
					<div className="border-r border-gray-300/50 pr-6">
						...
						<div className="mt-5 flex flex-col gap-3 border-t border-accent-600/20 pt-4">
							<Link
								to="/update"
								className="border-2 border-accent-600 text-on-primary px-4 py-1.5 rounded-xl text-sm text-center bg-accent-700 transition-colors"
							>
								Cập nhật phần mềm
							</Link>
							<Link
					...
					<div>
						...
						<div className="flex flex-col gap-4">
							{popularSearches.length > 0 ? (
								popularSearches.map((item, i) => (
									<Link
										key={i}
										to={item.path}
										className="font-medium hover:underline text-sm hover:text-accent-600 transition-colors"
									>
										{item.name}
									</Link>
								))
							...
							<Link
								to="/tat-ca"
								className="text-accent-600 text-sm mt-2 flex items-center gap-1 hover:underline"
							>
								Xem thêm &gt;
							</Link>
						</div>

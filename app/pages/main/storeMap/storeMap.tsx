...
export default function StoreMapPage(): JSX.Element {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      STORE_LOCATION.forEach((store) => {
        const dist = Math.sqrt(
          Math.pow(store.lat - latitude, 2) + Math.pow(store.lng - longitude, 2)
        );
        if (dist < minDistance) {
          minDistance = dist;
          nearestStore = store;
  ...
  useEffect(() => {
    ...
    STORE_LOCATION.forEach((store: StoreMap) => {
      ...
      popupEl.className = "w-64 rounded-lg overflow-hidden shadow-xl bg-white border border-gray-100";
      popupEl.innerHTML = `
  ...
  <div class="p-3">
    ...
    <button id="btn-maps-${store.id}" class="w-full mt-3 bg-accent-600 text-white text-xs py-2 rounded flex items-center justify-center gap-1 hover:bg-accent-700 transition-colors">
       Google Maps
    </button>
  </div>
`;

      popupEl.querySelector(`#btn-maps-${store.id}`)?.addEventListener("click", () => openGoogleMaps(store));
  ...
  const handleFocusStore = (store: StoreMap): void => {
    ...
    mapRef.current.flyTo({ center: [store.lng, store.lat], zoom: 15, essential: true });
  ...
  return (
    <div className="lg:w-10/12 w-11/12 mx-auto lg:py-10  py-5">
      <div className="  mb-2">
        <h1 className="text-2xl lg:text-3xl font-bold text-accent-600">Hệ Thống Cửa Hàng</h1>
          ...
          <div className="flex flex-col gap-4 overflow-y-auto max-h-[45vh] lg:max-h-[600px] custom-scroll-x pr-1">
            {STORE_LOCATION.map((store) => (
              <div key={store.id} className="relative group">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); openGoogleMaps(store); }}
                  className="absolute bottom-4 right-4 bg-white/90 p-2 rounded-full shadow-md hover:bg-accent-600  transition-all lg:opacity-0 group-hover:opacity-100"
                  title="Mở Google Maps"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /></svg>
                </button>
              </div>
            ))}
          </div>
